...
_PG_init(void)
{
    ...
    if(!process_shared_preload_libraries_in_progress){
        elog(FATAL, "pg_leader - please use shared_preload_libraries");
    }
...
shmem_request(void)
{
    ...
    //RequestNamedLWLockTranche("node shared part", 1);
}
...
shmem_startup(void)
{
    ...
    if(!found) {
        ...
        //shared_info_node->lock = &(GetNamedLWLockTranche("node shared part"))->lock;
    }
